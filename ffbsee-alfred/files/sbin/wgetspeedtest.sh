...
FASTDSOCK="$(uci -q get fastd.mesh_vpn.status_socket 2> /dev/null)"
MYFFGW=$(sockread $FASTDSOCK < /dev/null 2> /dev/null | sed 's/\(.*\)"name": "[a-zA-Z_]*gw\([0-9]*\)[^"]*"\(.*\)established\(.*\)/\2/g')
MYNAME="$(uci -q get system.@system[0].hostname 2> /dev/null)"
